...
def gen(seed: np.ndarray, base: ModuloBase, n: int, _l: int, epsilon_q: int) -> np.ndarray:
    # todo: check if seed is of length n, or trim/extend it to size
    byte_seed = np.packbits(seed.reshape((-1, 8))).tobytes()
    # todo: replace SHAKE from lib with full implementation?
    shake = SHAKE128.new()
...
def key_gen(n: int = 256, _l: int = 3, epsilon_p: int = 10, epsilon_q: int = 13,
            mi: int = 8) -> Tuple[np.ndarray, Tuple[np.ndarray, np.ndarray]]:
